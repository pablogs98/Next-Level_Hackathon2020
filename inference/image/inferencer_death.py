...
def main(args) :
	...
	stream_death = ibm_cos.Object(config['Bucket'], 'model_death.pickle').get()['Body']
	model_death = pickle.loads(stream_death.read())

	death_value = model_death.predict(user_profile)[0]

	# return {"body":"<html><body><h3>{}</h3></body></html>".format(death_value)}
	return { "body": { "death_value" : death_value }}
